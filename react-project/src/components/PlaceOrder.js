...
import { setUserEmail, resetCartDetails, setUserDetails } from '../redux/actions/CartAction';
...
export class PlaceOrder extends Component {
  ...
  componentDidMount() {
    ...
    console.log("In place order ", email);
    let request = {
      cart: this.props.cart,
      email: email,
      qrCode: qrCodeURL
    };
    axios.post("http://localhost:3000/mailserver", request).then(response => {
      let { data } = response;
      console.log(data);
    ...
    return (
      <div>
        ...
        <div className="container">
          Your Generated QR Code is as follows for unique delivery.
          The Mail has been sent to you for confirmation.
          <QRCode
            id="qrCode"
            size={150}
            value={qrCodeText}
